...
// ── 3) 용신 세력 곡선
// 곡선(순수 세력 기준): 적정 만점 구간 30~45%, 과다는 하락
ok('yongScore(0) = 0', M.yongScore(0) === 0);
ok('yongScore(0.35) = 20 (적정 만점)', M.yongScore(0.35) === 20, M.yongScore(0.35));
ok('yongScore(0.22) < 20 (월지 1글자 수준은 만점 아님)', M.yongScore(0.22) < 20, M.yongScore(0.22));
...
// ── 4) elemRatio: 전체 합 = 1, 극단 사주
{
  ...
  const b3 = { ys: 8, ms: 2, ds: 0, hs: 2, yb: 2, mb: 0, db: 2, hb: 2 }; // 수 = 월지20+년간7 = 27/92 (자리가중)
  const r = M.elemRatio(b3, 4);
  ok('월지 수 사주 → 수 비중(자리가중) ≈ 29%', Math.abs(r - 27 / 92) < 1e-9, r);
  ok('수 2글자(월지+년간) → 세력 만점(29%~) 근처지만 곡선상 만점', M.yongScore(r) === 20, M.yongScore(r));
  // 월지 1글자만인 경우: 세력이 만점 문턱(30%) 아래로 내려가 만점 아님
  const b1 = { ys: 0, ms: 2, ds: 0, hs: 2, yb: 2, mb: 0, db: 2, hb: 1 }; // 수 = 월지20 뿐 = 20/92 ≈ 22%
  ok('월지 수 1글자뿐 → 세력 22% → 용신점수 만점 미만', M.yongScore(M.elemRatio(b1, 4)) < 20, M.yongScore(M.elemRatio(b1, 4)));
}
// ── 5) 대운: 방향·대운수·첫 대운
{
